...
export default function SellerDashboardClient({
  ...
  const maxCategoryRevenue = Math.max(
    ...categoryBreakdown.map((c) => c.revenue),
    1
  )
    ...
    orders.forEach((order) => {
      order.items.forEach((item) => {
        if (!map[item.productId])
          map[item.productId] = { name: item.name, units: 0, revenue: 0 }
  ...
  const filteredListings = useMemo(() => {
    ...
    return listings.filter((l) =>
      [l.name, l.category, l.status, ...(l.materials ?? [])]
        .join(" ")
        .toLowerCase()
        .includes(q)
    )
  ...
  const uploadImages = async () => {
    ...
    if (!imageFiles.length)
      return { image: draft.image, galleryImages: draft.galleryImages ?? [] }
    ...
    <>
      ...
      <section className="overflow-hidden rounded-lg border border-[#d8dfdc] bg-white shadow-sm">
        ...
        <div className="px-5 pb-5">
            ...
            <div className="flex items-end gap-4">
              ...
              <div className="pb-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="rounded bg-[#edf7f4] px-2 py-0.5 text-[10px] font-black tracking-wide text-[#063f34] uppercase">
                    Verified Seller
                  </span>
                </div>
          ...
          {user.story && (
            <p className="mt-3 max-w-2xl text-sm leading-6 text-[#53615c]">
              {user.story}
            </p>
          )}
        </div>
        ...
        <div className="mt-4 flex items-start gap-3 rounded-lg border border-[#fde68a] bg-[#fffbeb] px-4 py-3">
          ...
          <div>
            <p className="text-sm font-bold text-[#78350f]">
              {lowStock.length} listing{lowStock.length > 1 ? "s" : ""} running
              low
            </p>
            <p className="mt-0.5 text-xs text-[#92400e]">
              {lowStock.map((l) => l.name).join(", ")} — update quantities
            ...
            {statCards.map(({ label, value, hint, icon: Icon, accent, bg }) => (
              ...
              >
                ...
                <div className="p-4">
                  ...
                  <p className="mt-3 text-[10px] font-bold tracking-widest text-[#9aada8] uppercase">
                    {label}
                  </p>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <section className="overflow-hidden rounded-lg border border-[#d8dfdc] bg-white shadow-sm">
              <div className="border-b border-[#e8edeb] bg-[#f9faf9] px-5 py-4">
                ...
                <h2 className="mt-1 text-base font-black text-[#063f34]">
                  Top Products
                </h2>
              </div>
              <div className="divide-y divide-[#eef1ef]">
                {isLoading ? (
                  ...
                  topProducts.map((product, i) => (
                    <div
                      key={product.name}
                      className="flex items-center gap-3 px-5 py-3 hover:bg-[#f9faf9]"
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-bold text-[#191c1c]">
                          {product.name}
                        </p>
                      ...
                      <p className="text-sm font-black text-[#063f34]">
                  ...
                  <div className="flex flex-col items-center justify-center gap-2 py-10 text-center">
                    ...
                    <p className="text-sm font-semibold text-[#9aada8]">
                      No sales yet
                    </p>
                    <p className="text-xs text-[#bfc9c4]">
                      Revenue by product will appear after your first order
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4 p-5">
                {isLoading ? (
                  ...
                  categoryBreakdown.map(({ cat, revenue, count }) => (
                    <div key={cat}>
                      <div className="mb-1.5 flex items-center justify-between">
                        ...
                        <span className="text-xs font-bold text-[#063f34]">
                          ${revenue.toFixed(2)}
                        </span>
                      </div>
                      ...
                      <p className="mt-1 text-[10px] text-[#9aada8]">
                  ...
                  <div className="flex flex-col items-center justify-center gap-2 py-8 text-center">
                    ...
                    <p className="text-xs text-[#bfc9c4]">
                      Category breakdown appears after your first sale
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="flex flex-col gap-3 border-b border-[#e8edeb] bg-[#f9faf9] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex gap-2">
                ...
                <div className="relative">
                  <Search
                    className="absolute top-1/2 left-2.5 -translate-y-1/2 text-[#9aada8]"
                    size={13}
                  />
                  <input
                    value={listingQuery}
                    ...
                    className="h-9 w-48 rounded-lg border border-[#d8dfdc] bg-white pr-3 pl-8 text-sm font-medium transition-all outline-none placeholder:text-[#bfc9c4] focus:border-[#063f34] focus:ring-4 focus:ring-[#063f34]/8"
                  />
                </div>
                <button
                  onClick={openAddModal}
              ...
              <table className="w-full min-w-[640px] text-left text-sm">
                <thead>
                  <tr className="border-b border-[#e8edeb] bg-[#f4f6f5]">
                    {["Product", "Status", "Stock", "Price", "Actions"].map(
                      (h) => (
                        <th
                          key={h}
                          className="px-4 py-3 text-[10px] font-black tracking-widest text-[#9aada8] uppercase"
                        >
                          {h}
                        </th>
                      )
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs font-medium tracking-wide text-[#9aada8] uppercase">
                                {product.category}
                                {product.materials?.[0]
                                  ? ` · ${product.materials[0]}`
                                  : ""}
                        ...
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <p className="text-xs text-[#bfc9c4]">
                            {listingQuery
                              ...
                              : "Add your first product to publish it to the marketplace"}
                          </p>
                          {!listingQuery && (
                            <button
                              onClick={openAddModal}
      ...
      >
        <DialogContent className="max-h-[calc(100dvh-2rem)] max-w-[760px] gap-0 overflow-hidden rounded-lg border-[#cfd9d4] bg-[#f4f6f5] p-0 text-[#191c1c] shadow-2xl sm:max-w-[760px]">
          <form
            ref={formRef}
          ...
          >
            <div className="border-b border-[#d8dfdc] bg-white px-5 py-4">
              <DialogHeader className="pr-10">
                <p className="text-[10px] font-black tracking-widest text-[#9a4d10] uppercase">
            ...
            <div className="grid gap-4 overflow-y-auto bg-[#f4f6f5] p-5 sm:grid-cols-2">
              ...
              <label className="sm:col-span-2">
                ...
                <textarea
                  ...
                  rows={4}
                  className={`w-full rounded-lg border bg-white px-3 py-2 text-sm transition-all outline-none focus:border-[#063f34] focus:ring-4 focus:ring-[#063f34]/8 ${errors.description ? "border-[#ba1a1a]" : "border-[#d8dfdc]"}`}
                />
              ...
              {formMessage && (
                <Alert
                  variant="destructive"
                  className="rounded-lg border-[#f0b8b8] bg-[#fff7f7] text-[#7a1d1d] sm:col-span-2"
                >
                  <Info />
            ...
            <div className="flex justify-end gap-2 border-t border-[#d8dfdc] bg-white px-5 py-4">
              <button
                type="button"
                onClick={closeModal}
                className="rounded-lg border border-[#d8dfdc] px-4 py-2 text-sm font-bold text-[#53615c] transition-colors hover:bg-[#f4f6f5]"
              >
                Cancel
              </button>
              <button
      ...
      >
        <DialogContent className="max-w-[420px] overflow-hidden rounded-lg border-[#f0b8b8] bg-white p-0 text-[#191c1c]">
          <div className="bg-[#fff7f7] px-6 pt-6 pb-4">
            ...
            <DialogHeader>
              <DialogTitle className="text-base font-black text-[#191c1c]">
                Delete product?
              </DialogTitle>
          ...
          <div className="flex flex-col-reverse gap-2 px-6 py-4 sm:flex-row sm:justify-end">
            <button
              ...
              disabled={isDeleting}
              className="rounded-lg border border-[#d8dfdc] px-4 py-2 text-sm font-bold text-[#53615c] transition-colors hover:bg-[#f4f6f5] disabled:opacity-70"
            >
              Cancel
            </button>
            <button
              ...
              disabled={isDeleting}
              className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#ba1a1a] px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-[#9e1515] disabled:opacity-70"
            >
  ...
  if (!orders.length) {
    return (
      <div className="flex flex-col items-center justify-center gap-2 py-14 text-center">
        ...
        <p className="text-xs text-[#bfc9c4]">
          Orders for your studio will appear here after checkout
        </p>
      </div>
    )
  }
        ...
        return (
          <div key={order.id}>
            ...
            >
              ...
              <p className="shrink-0 text-sm font-black text-[#191c1c]">
            ...
            {isExpanded && (
              <div className="border-t border-[#eef1ef] bg-[#f9faf9] px-5 py-3">
                <div className="ml-0 overflow-hidden rounded-lg border border-[#e8edeb] bg-white">
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-[#eef1ef] bg-[#f4f6f5]">
                        <th className="px-4 py-2 text-left text-[10px] font-black tracking-widest text-[#9aada8] uppercase">
                          Item
                        </th>
                        <th className="px-4 py-2 text-center text-[10px] font-black tracking-widest text-[#9aada8] uppercase">
                          Qty
                        </th>
                        <th className="px-4 py-2 text-right text-[10px] font-black tracking-widest text-[#9aada8] uppercase">
                          Subtotal
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[#eef1ef]">
                      {order.items.map((item) => (
                        <tr key={item.productId}>
                          <td className="px-4 py-2.5 font-medium text-[#191c1c]">
                            {item.name}
                          </td>
                          <td className="px-4 py-2.5 text-center text-[#53615c]">
                            {item.quantity}
                          </td>
                    ...
                    <tfoot>
                      <tr className="border-t border-[#e8edeb] bg-[#f9faf9]">
                        <td
                          colSpan={2}
                          className="px-4 py-2 text-right text-xs font-black tracking-wide text-[#9aada8] uppercase"
                        >
                          Order total
                        </td>
                        <td className="px-4 py-2 text-right font-black text-[#063f34]">
                          ${orderTotal.toFixed(2)}
                        </td>
                      </tr>
                    </tfoot>
...
function ListingStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`rounded-md px-2.5 py-0.5 text-xs font-bold ${styles[status] ?? "bg-[#f4f6f5] text-[#6d7a75]"}`}
    >
      {status}
    </span>
  )
...
function OrderStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 rounded-md px-2.5 py-0.5 text-xs font-bold ${styles[status] ?? "bg-[#f4f6f5] text-[#6d7a75]"}`}
    >
...
function ProductImageUpload({
  ...
  return (
    <label className="sm:col-span-2">
      ...
      <div
        className={`grid gap-4 rounded-lg border bg-white p-3 sm:grid-cols-[160px_1fr] ${error ? "border-[#ba1a1a]" : "border-[#d8dfdc]"}`}
      >
        <div className="relative aspect-[4/3] overflow-hidden rounded-lg bg-[#f4f6f5]">
          {displayImage ? (
            ...
            <div className="flex h-full items-center justify-center px-3 text-center text-xs font-semibold text-[#9aada8]">
            ...
            <div className="mt-3 grid grid-cols-5 gap-2">
              {previewImages.slice(1, 6).map((previewImage, index) => (
                <div
                  ...
                  className="relative aspect-square overflow-hidden rounded bg-[#f4f6f5]"
                >
                  <ShimmerImage
                    src={previewImage}
                    alt=""
  ...
  return (
    <label className={className}>
      ...
      <input
        ...
        className={`h-9 w-full rounded-lg border bg-white px-3 text-sm font-medium transition-all outline-none focus:ring-4 ${
          error
      ...
      {error ? (
        ...
        <span id={helpId} className="mt-1 block text-xs text-[#9aada8]">
          {helper}
        </span>
      ) : null}
    </label>
    ...
    <label>
      ...
      <Select value={value} onValueChange={onChange}>
        <SelectTrigger
          ...
          className={`w-full bg-white text-sm font-medium text-[#191c1c] ${error ? "border-[#ba1a1a]" : "border-[#d8dfdc]"}`}
        >
        ...
        <SelectContent className="z-[120] border border-[#cfd9d4] bg-white text-[#191c1c] shadow-xl">
          {options.map((option) => (
            <SelectItem
              key={option}
              value={option}
              className="text-sm font-medium text-[#25332e] focus:bg-[#edf7f4] focus:text-[#063f34]"
            >
              {option}
            </SelectItem>
          ))}
