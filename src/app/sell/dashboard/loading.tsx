...
export default function SellerDashboardLoading() {
  return (
    <div className="min-h-screen bg-[#f4f6f5]">
      <Navbar />
      <main className="mx-auto max-w-[1160px] px-4 py-6 sm:px-5 lg:px-6 space-y-4">

        {/* Profile card skeleton */}
        <div className="overflow-hidden rounded-lg border border-[#d8dfdc] bg-white animate-pulse">
          <div className="h-36 bg-[#e8edeb] sm:h-44" />
          <div className="px-5 pb-5 pt-3">
            <div className="flex items-end gap-4">
              <div className="-mt-10 h-20 w-20 shrink-0 rounded-lg bg-[#e8edeb] border-4 border-white" />
          ...
          <div className="border-t border-[#e8edeb] px-5">
            <div className="flex gap-6 py-3">
              {[80, 64, 72].map((w) => (
                <div key={w} className="h-4 rounded bg-[#e8edeb]" style={{ width: w }} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="overflow-hidden rounded-lg border border-[#d8dfdc] bg-white animate-pulse" style={{ animationDelay: `${i * 60}ms` }}>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {[1, 2].map((i) => (
            <div key={i} className="rounded-lg border border-[#d8dfdc] bg-white overflow-hidden animate-pulse" style={{ animationDelay: `${i * 100}ms` }}>
              <div className="border-b border-[#e8edeb] bg-[#f9faf9] px-5 py-4 space-y-2">
