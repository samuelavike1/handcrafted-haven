        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
          <section className="space-y-5">
            {items.map((item) => (
              <article
                key={item.id}
                className="grid gap-5 rounded-lg border border-[#d8dfdc] bg-white p-4 sm:grid-cols-[128px_1fr_auto]"
              >
                ...
                <div>
                  ...
                  <h2 className="mt-1 text-xl font-black text-[#063f34]">
                    {item.name}
                  </h2>
                  <p className="mt-2 text-sm text-[#53615c]">
                    by <span className="font-bold">{item.seller}</span>
                  </p>
                  <p className="mt-4 max-w-xl text-sm leading-relaxed text-[#53615c]">
                    {item.description}
                  </p>
                </div>
                <div className="flex flex-row items-center justify-between gap-4 sm:flex-col sm:items-end">
                  <p className="text-xl font-black">${item.price.toFixed(2)}</p>
                  <div className="flex h-10 items-center rounded-lg border border-[#d8dfdc]">
                    <button
                      className="px-3"
          ...
          <aside className="h-fit rounded-lg border border-[#d8dfdc] bg-white p-5 shadow-[0_12px_28px_rgba(18,40,33,0.08)] lg:sticky lg:top-24">
            <h2 className="text-xl font-black text-[#063f34]">Order summary</h2>
            ...
            <div className="mt-6 border-t border-[#d8dfdc] pt-6">
              <div className="flex items-end justify-between">
                ...
                <span className="text-3xl font-black text-[#063f34]">
                  ${total.toFixed(2)}
                </span>
              </div>
            </div>
            <Link
              href="/checkout"
              className="mt-7 flex h-11 items-center justify-center rounded-lg bg-[#f28a35] font-black text-white transition hover:bg-[#dc7624]"
            >
