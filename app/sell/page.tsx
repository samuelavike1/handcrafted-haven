      ...
      <main className="mx-auto max-w-[1180px] px-4 py-8 sm:px-6 lg:px-8">
        <section className="overflow-hidden rounded-lg border border-[#d8dfdc] bg-white">
          ...
          <div className="grid gap-6 p-5 md:grid-cols-[1fr_auto] md:p-6">
            <div className="flex flex-col gap-5 sm:flex-row">
              <div className="relative -mt-16 h-28 w-28 shrink-0 overflow-hidden rounded-lg border-4 border-white shadow-xl">
                <Image
                  ...
                  unoptimized
                />
                <span className="absolute right-2 bottom-2 flex h-7 w-7 items-center justify-center rounded-lg bg-[#063f34] text-white">
                  <BadgeCheck size={17} />
                </span>
              </div>
              <div>
                <h1 className="text-3xl font-black text-[#063f34]">
                  {featuredSeller.name}
                </h1>
                ...
                <p className="mt-4 max-w-3xl leading-relaxed text-[#53615c]">
                  {featuredSeller.story}
                </p>
              </div>
            </div>
            <div className="flex gap-3 md:self-start">
              <button className="rounded-lg border border-[#063f34] px-4 py-2.5 text-sm font-black text-[#063f34]">
                Edit profile
              </button>
              <Link
                href="/browse"
                className="rounded-lg bg-[#f28a35] px-4 py-2.5 text-sm font-black text-white"
              >
                View public shop
              </Link>
            </div>
        ...
        <section className="mt-6 grid gap-4 md:grid-cols-4">
          {stats.map(({ label, value, hint, icon: Icon }, index) => (
            <div
              key={label}
              className={`rounded-lg border p-5 ${index === 0 ? "border-[#063f34] bg-[#063f34] text-white" : "border-[#d8dfdc] bg-white"}`}
            >
              <Icon
                size={22}
              ...
              />
              <p
                className={`mt-5 text-xs font-black uppercase ${index === 0 ? "text-white/62" : "text-[#9a4d10]"}`}
              >
                {label}
              </p>
        ...
        <section className="mt-6 rounded-lg border border-[#d8dfdc] bg-white">
          <div className="flex flex-col gap-4 border-b border-[#d8dfdc] p-5 sm:flex-row sm:items-center sm:justify-between">
            <div>
            ...
            <button className="inline-flex items-center justify-center rounded-lg bg-[#063f34] px-4 py-2.5 text-sm font-black text-white">
                  ...
                  <tr key={product.id} className="hover:bg-[#fbfbf8]">
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-4">
                        <div className="relative h-14 w-14 overflow-hidden rounded-lg bg-[#edf2ef]">
                          <Image
                            src={product.image}
                            alt=""
                            fill
                    ...
                    <td className="px-6 py-5 font-semibold text-[#53615c]">
